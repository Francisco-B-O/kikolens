...
def main():
    ...
    if st.session_state.df is not None:
        ...
        if page == "Vision":
            ...
            c1.metric("Rows", f"{len(df):,}"); c2.metric("Features", len(df.columns))
            c3.metric("Health", f"{(1 - df.isnull().sum().sum()/df.size)*100:.1f}%")
            ctrl, plot = st.columns([1, 3])
            x = ctrl.selectbox("X Axis", df.columns)
            y = ctrl.selectbox("Y Axis", df.columns, index=min(1, len(df.columns)-1))
            ...
            with tab2:
                ...
                else: st.info("No numeric columns available for correlation analysis.")
            ...
            if mode == "🛠️ Manual":
                manual_type = st.selectbox("Algorithm", ["Random Forest", "Gradient Boosting", "Decision Tree", "Ridge/Logistic", "Lasso/KNN"])
            elif mode == "📤 Import":
            ...
            train_triggered = c_btn.button("🚀 Train Pipeline")
                    ...
                    if not top_f:
                        ...
                        for i, f in enumerate(top_f):
                            ...
                            if bf:
                                ...
                                col_min, col_max = float(df[f].min()), float(df[f].max())
                                # BUG F FIX: usar number_input si min == max (columna constante)
                                if col_min < col_max:
                                    sim_v[f] = sim_cols[i].slider(f, col_min, col_max, float(df[f].mean()))
                                else:
                                    sim_v[f] = sim_cols[i].number_input(f, value=col_min)
        ...
        st.markdown("<div style='text-align:center; padding-top:100px;'><h1>KIKOLENS</h1><p>Connect your data via the sidebar to start.</p></div>", unsafe_allow_html=True)
